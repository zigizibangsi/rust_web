#![warn(clippy::all)]
use clap::Parser; //clap 분석기를 가져온다. 해당 분석기는 명령줄에서 인수를 분석해 Args 객체로 변환해 준다.
...
async fn main() -> Result<(), handle_errors::Error> {
    ...
    .await
    .map_err(|e| handle_errors::Error::DatabaseQueryError(e))?; // 데이터베이스 풀 생성에서 반환하는 기본 에러를 남은 코드베이스와 일치하도록 변경하고, handle-errors 크레이트의 Error 열거 값의 하나로 반환한다.

    sqlx::migrate!()
        .run(&store.clone().connection)
        .await
        // .expect("Cannot run migration");
        .map_err(|e| handle_errors::Error::MigrationError(e))?; // 어떤 이유로든 마이그레이션이 실패하면 반환할 새로운 MigrationError 열거 값을 추가한다.
    ...
    // warp::serve(routes).run(([127, 0, 0, 1], args.port)).await; // 서버를 시작하기 위해 파싱한 PORT 환경 변수를 사용한다.
    tracing::info!("Q&A service build ID {}", env!("RUST_WEB_DEV_VERSION"));

    warp::serve(routes).run(([127, 0, 0, 1], config.port)).await; // 서버를 시작하기 위해 파싱한 PORT 환경 변수를 사용한다.
